...
const BookingModal = ({ isOpen, onClose, dress, service }) => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <button
          ...
          className={`
            relative p-4 rounded-2xl font-black transition-all flex flex-col items-center justify-center gap-1
        ...
        >
          ...
          {isBooked && (
             <div className="absolute inset-0 flex items-center justify-center">
                <div className="w-10 h-10 border-2 border-rose-600 rounded-full flex items-center justify-center animate-pulse">
                   <div className="w-8 h-1 bg-rose-600 rotate-45 absolute"></div>
                   <div className="w-8 h-1 bg-rose-600 -rotate-45 absolute"></div>
                </div>
             </div>
          )}
        ...
        return (
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-left duration-500">
            <div className="flex justify-between items-center">
              <h3 className="text-xl font-black text-slate-900">اختاري تاريخ المناسبة</h3>
              <div className="flex items-center gap-4 bg-slate-100 p-2 rounded-2xl">
                 <button type="button" onClick={prevMonth} className="p-2 hover:bg-white rounded-xl transition-all"><ChevronRight size={20} /></button>
                 <span className="font-black text-slate-800 min-w-[120px] text-center">
                   {currentDate.toLocaleDateString('ar-YE', { month: 'long', year: 'numeric' })}
                 </span>
                 <button type="button" onClick={nextMonth} className="p-2 hover:bg-white rounded-xl transition-all"><ChevronLeft size={20} /></button>
              </div>
            </div>
            ...
            <div className="flex gap-6 items-center bg-slate-50 p-6 rounded-3xl border border-slate-100">
              <div className="flex items-center gap-2 text-xs font-bold text-slate-500">
                 <div className="w-3 h-3 bg-rose-600 rounded-full"></div> المختار
              </div>
              <div className="flex items-center gap-2 text-xs font-bold text-slate-500">
  ...
  return (
    <div className="fixed inset-0 z-100 flex items-center justify-center px-6">
      ...
      <div className="relative bg-white w-full max-w-xl rounded-4xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
        {success ? (
          ...
          <>
            <div className="bg-slate-900 p-8 text-white">
              <div className="flex justify-between items-start">
                <div className="flex-2 text-right">
                  <h2 className="text-3xl font-black mb-1">تأكيد الحجز</h2>
                  <p className="text-slate-400 font-medium">الخطوة {step} من 3</p>
                </div>
                <button 
                  onClick={onClose}
            ...
            <form onSubmit={handleBooking} className="p-8 lg:p-10 space-y-8">
              {/* Step indicator header */}
              <div className="flex items-center gap-4 text-right mb-4">
                <div className="bg-rose-100 text-rose-600 w-10 h-10 rounded-xl flex items-center justify-center font-black">
                  {step === 1 ? '١' : step === 2 ? '٢' : '٣'}
                </div>
                <div>
                  ...
                  <p className="text-xs text-slate-400 font-bold">يرجى ملء البيانات بدقة</p>
                </div>
              </div>
              ...
              <div className="flex gap-4">
                {step > 1 && (
                  <button 
                    type="button"
                    onClick={prevStep}
                    className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 py-5 rounded-3xl text-lg font-black transition-all"
                  >
                    السابق
                  </button>
                )}
                <button 
                  type="submit"
                  disabled={loading}
                  className="flex-[2] bg-rose-600 hover:bg-rose-700 text-white py-5 rounded-3xl text-lg font-black shadow-xl shadow-rose-200 transition-all flex items-center justify-center gap-3 hover:-translate-y-1 active:scale-95 disabled:opacity-50"
                >
                  {loading ? (
